...
void MainWindow::save(QUrl url)
{
    ...
    if(!url.isValid() || input.isNull()) return;

    QString path = url.toLocalFile();

    //if no file suffix was chosen, automatically use the PNG format
    QFileInfo file(path);
    if(!file.baseName().isEmpty() && file.suffix().isEmpty()) path += ".png";

    QString suffix = file.suffix();
    //Qt can only read tga, saving is not supported
    if(suffix.toLower() == "tga") suffix = "png";
    ...
    if(successfullySaved) ui->statusBar->showMessage("Maps successfully saved", 4000);
    else QMessageBox::information(this, "Maps not saved", "One or more of the maps was NOT saved!");
...
void MainWindow::changeOutputPathQueueDialog()
{
    ...
    if(exportPath.isValid()) startUrl = exportPath;

    QUrl path = QFileDialog::getExistingDirectoryUrl(this, "Choose Export Folder", startUrl);
    setExportPath(path);
}
